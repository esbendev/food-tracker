(function () {
  ...
  if (!record) {
    window.alert("No se encontro la comida que queres editar.");
    redirectHome();
    return;
  }

  backLink.href = buildHomeHref();
  dateLabel.textContent = "Editando comida de " + app.prettyDate(record.date);
  app.setSelectedMeal("meal", record.meal);
  record.items.forEach(function (value) {
    addInputField(value);
  });
